...
from dragon_chat_agent.utils.langchain_langfuse.utils import _extract_model_name
...
try:
    ...
    logger.error(f"Could not import langchain. The langchain integration will not work. {e}")
    langfuse_logger.error(f"Could not import langchain. The langchain integration will not work. {e}")
    raise ModuleNotFoundError(
        "Please install langchain to use the Langfuse langchain integration V22: 'pip install langchain' {e}"
    )
...
class LangchainCallbackHandler(LangchainBaseCallbackHandler):
    ...
    def _detach_observation(
        self, run_id: UUID
    ) -> Union[LangfuseAgent, LangfuseChain, LangfuseGeneration, LangfuseRetriever, LangfuseSpan, LangfuseTool] | None:
        ...
        return cast(
            "Union[LangfuseAgent, LangfuseChain, LangfuseGeneration, LangfuseRetriever, LangfuseSpan, LangfuseTool]",
